...
async function getImage(client, res, emoji, user, history = false) {  
  ...
  var output = {}
  if (!(history)) {
    dbo.collection("images").find(q).sort({timestamp: -1}).limit(250).toArray(function(err, result) {
      if (err) throw err;
      output = getPreference(result)
      returnJSON(output, res)
    })
  } else {
    dbo.collection("images").sample(q).sort({timestamp: -1}).limit(250).toArray(function(err, result) {
      if (err) throw err;
      output = getPreference(result)
      returnJSON(output, res)
    })
